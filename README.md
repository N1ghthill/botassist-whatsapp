...
**Open-source focado em automação operacional e produtividade de atendimento.**
...
- Impacto esperado: menor tempo de resposta, mais consistência no atendimento e operação sustentável 24/7.

## Quem usa (preencher com métricas reais)
- Usuários ativos por mês: `TODO`.
- Ambientes em operação (pessoal, negócio local, equipe): `TODO`.
- Taxa de resolução sem intervenção humana: `TODO`.
...
- Roadmap técnico: `docs/`

## Skills / Keywords (SEO)
`automation`, `ML`, `open-source`, `API`, `Node.js`, `TypeScript`, `Electron`, `WhatsApp`, `AI assistant`

## Disponibilidade
Disponível para consultoria em automação conversacional, desenho de produto de IA e liderança técnica (CTO fractional).
